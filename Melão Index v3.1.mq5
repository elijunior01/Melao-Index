...
// Usa tempo relativo ao primeiro ponto para maior estabilidade numérica.
                            ...
                            bool ponderado = false, double peso_base = 0.95)
{
   ...
   if(ponderado)
   {
      if(peso_base <= 0.0 || peso_base > 1.0)
         peso_base = 0.95;
   }
   ...
   for(int i = 0; i < n; i++)
   {
      ...
      if(ponderado)
      {
         // mais recente = maior peso
         pesos[i] = MathPow(peso_base, n - 1 - i);
         soma_pesos += pesos[i];
      }
      else
      {
...
double TransformarParaMDDestrela(double fracao_mdd)
{
   ...
   if(fracao_mdd >= 0.999999)
      return 1e12;  // evita divisão por zero

   return fracao_mdd / (1.0 - fracao_mdd);
}

// Considera como trading apenas BUY/SELL.
// Os demais tipos são tratados como movimentos não-trading.
...
// Lucro do deal (mantido conforme a base original: DEAL_PROFIT)
double LucroDeal(ulong ticket)
{
...
bool ConstruirSerieSaldos(SerieTemporal &st,
                          ...
                          datetime fim,
                          int passo_segundos,
                          double saldo_inicial_param,
                          double &saldo_base_usado,
                          double &lucro_trading_total,
                          double &movimentos_externos_total,
                          int &deals_considerados,
                          int &deals_ignorados)
{
   ...
   saldo_base_usado = 0.0;
   lucro_trading_total = 0.0;
   movimentos_externos_total = 0.0;
   deals_considerados = 0;
   deals_ignorados = 0;
   ...
   // Se início não fornecido, busca o deal trading mais antigo nos últimos 10 anos
   if(inicio == 0)
   {
      datetime probe_inicio = TimeCurrent() - 10 * 365 * 24 * 3600;
   ...
   for(int i = 0; i < total_deals; i++)
   {
      ...
      double resultado = LucroDeal(ticket);
      ...
      if(DealEhTrading(ticket))
      {
         ...
         soma_lucro_bucket[idx] += resultado;
         lucro_trading_total += resultado;
         deals_considerados++;
      }
      else
      {
         // Movimento externo / não-trading
         movimentos_externos_total += resultado;
         deals_ignorados++;
      }
   }
   ...
   if(saldo_inicial_param > 0.0)
   {
      saldo_base_usado = saldo_inicial_param;
   }
   else
   {
      double saldo_atual = AccountInfoDouble(ACCOUNT_BALANCE);

      // Reconstrução aproximada do saldo base da série.
      // Mantém a teoria intacta e apenas ajusta a engenharia.
      saldo_base_usado = saldo_atual - lucro_trading_total - movimentos_externos_total;
   }

   if(saldo_base_usado <= 0.0)
      saldo_base_usado = EPSILON;

   // Caminha pelos buckets construindo a série
   double saldo_anterior = saldo_base_usado;
...
// Ordena DDItem[] por MDD* decrescente
void OrdenarDDItensDesc(DDItem &itens[], int n)
{
   if(n < 2)
      return;
   ...
   for(int i = 0; i < n - 1; i++)
   {
      ...
      {
         if(itens[j].mdd_star < itens[j + 1].mdd_star)
         {
            DDItem temp = itens[j];
            itens[j] = itens[j + 1];
            itens[j + 1] = temp;
...
void OnStart()
{
   Print("Início do script Índice Melão v3.12.");
   ...
   double fator_ponderacao_usado = fator_ponderacao;
   if(ponderar_regressao && (fator_ponderacao_usado <= 0.0 || fator_ponderacao_usado > 1.0))
   {
      Print("Aviso: fator_ponderacao fora do intervalo (0,1]. Ajustando para 0.95.");
      fator_ponderacao_usado = 0.95;
   }

   if(tempo_inicio != 0 && saldo_inicial_manual <= 0.0)
   {
      Print("Aviso: com tempo_inicio explícito e saldo_inicial_manual=0, o saldo base é aproximado.");
      Print("Para máxima precisão, informe o saldo inicial manual.");
   }
   ...
   double saldo_base_usado = 0.0;
   double lucro_trading_total = 0.0;
   double movimentos_externos_total = 0.0;
   int deals_considerados = 0;
   int deals_ignorados = 0;

   if(!ConstruirSerieSaldos(st, inicio, fim, segundos_periodo, saldo_inicial_manual,
                            saldo_base_usado, lucro_trading_total, movimentos_externos_total,
                            deals_considerados, deals_ignorados))
   {
   ...
   if(!RegressaoLinearLnSaldo(st.tempos, st.saldos, n, inclinacao_ano, intercepto,
                              ponderar_regressao, fator_ponderacao_usado))
   {
   ...
   DDItem dd_itens[];
   ArrayResize(dd_itens, quantidade_dd);

   for(int i = 0; i < quantidade_dd; i++)
   {
      dd_itens[i].mdd = dd_frac[i];
      dd_itens[i].mdd_star = TransformarParaMDDestrela(dd_frac[i]);
   }

   if(quantidade_dd > 1)
      OrdenarDDItensDesc(dd_itens, quantidade_dd);

   double MDDestrela_para_MeI = 0.0;
   double global_MDDestrela = (quantidade_dd > 0) ? dd_itens[0].mdd_star : 0.0;

   if(usarMediaParaMDDestrela)
   {
      int usar_k = (topK < quantidade_dd ? topK : quantidade_dd);
      if(usar_k > 0)
      {
         double soma = 0.0;
         for(int i = 0; i < usar_k; i++)
            soma += dd_itens[i].mdd_star;
   ...
   // Cálculo do MeI (fórmula do artigo)
   double ln_num = 0.0, ln_infl = 0.0, ln_denom = 0.0;
   ...
   saida += StringFormat("Deals de trading considerados: %d\n", deals_considerados);
   saida += StringFormat("Movimentos não-trading ignorados: %d\n", deals_ignorados);
   saida += StringFormat("Saldo base usado: %.8f\n", saldo_base_usado);
   saida += StringFormat("Lucro trading total no período: %.8f\n", lucro_trading_total);
   saida += StringFormat("Movimentos externos totais no período: %.8f\n", movimentos_externos_total);
   saida += "\n";
   ...
   for(int i = 0; i < quantidade_dd; i++)
      saida += StringFormat("  [%d] MDD=%.4f%%, MDD*=%.8f\n", i + 1, dd_itens[i].mdd * 100.0, dd_itens[i].mdd_star);
   ...
   if(tempo_inicio != 0 && saldo_inicial_manual <= 0.0)
      saida += "- Aviso: saldo inicial automático pode ficar aproximado quando tempo_inicio é definido manualmente.\n";

   if(saldos_corrigidos > 0)
      saida += StringFormat("- %d saldos negativos/nulos foram ajustados para EPSILON para permitir o log.\n", saldos_corrigidos);
